...
bool SegMatchWorker::processLocalMap(
    segmatch::SegMatch::LocalMapT& local_map, // 即typedef LocalMap<PclPoint, MapPoint> LocalMapT 与维护的LocalMap类型相同
    const laser_slam::Pose& latest_pose, //最新位姿
    unsigned int track_id, //机器人编号
    RelativePose* loop_closure) { //回环检测结果（没有这个参数表示不是回环检测人物）
  ...
  if ((params_.localize && target_cloud_loaded_) || params_.close_loops) {
    ...
    // 距离上次segmentation，机器人行驶过一定距离才进行后续处理
	  // last_segmented_poses_存储每个worker的最新位置与id对应关系
    bool robot_drove_enough = false;
    if (last_segmented_poses_.empty()) {
      //若未进行过segmentation，则进行segmentation并记录此时的pose和track _id
      ...
      for (auto& pose_track_id_pair: last_segmented_poses_) {
        //找这个track_id下上次进行segmentation时的pose
        if (pose_track_id_pair.second == track_id) {
          //这个robot曾进行过segmentation
          last_segmented_pose_set = true;
          //判断当前pose和上次segmentation的pose之间的距离，距离足够(yaml里设置的是0)再进行segmentation
      ...
      if (!last_segmented_pose_set) {
         //这个robot未曾进行过segmentation
        robot_drove_enough = true;
        last_segmented_poses_.push_back(PoseTrackIdPair(latest_pose, track_id));
      }
    }
    //若机器人运行距离不够，则不进行segmentation，返回false，表示未找到回环
