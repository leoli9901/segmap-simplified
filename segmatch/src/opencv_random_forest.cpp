    ...
    //source_segment即用于匹配的segment
     ...
     // 分割source_segment最新观测的特征矩阵，按顺序转换为一维矩阵（行向量）
      ...
      if (params_.normalize_eigen_for_knn) {
        ...
        //不归一化
        //block()函数是指取其前几维（knn_feature_dim定义）作为构建knn的特征向量q
        q = features_source.block(0, 0, 1, params_.knn_feature_dim).transpose()
            .cast<float>();
      }

     //最近邻的选取的数量由n_nearest_neighbours=120和target map中segment的数量中的最小值决定
      const unsigned int n_nearest_neighbours = std::min(
          params_.n_nearest_neighbours, int(target_segment_ids_.size()) - 1);
      // Eigen库 创建一个n_nearest_neightbours维的int类型向量indices
      VectorXi indices(n_nearest_neighbours);
      //Eigen库 创建一个n_nearest_neightbours维的float类型向量dist2
      ...
      for (size_t i = 0u; i < n_nearest_neighbours; ++i) {
        ...
        //排除自己？？？为什么target map里会有source segment？？？
