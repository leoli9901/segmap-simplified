...
LocalMap<InputPointT, ClusteredPointT>::LocalMap(
  ...
  : voxel_grid_(params.voxel_size_m, params.min_points_per_voxel) //体素滤波的分辨率
  , radius_squared_m2_(pow(params.radius_m, 2.0)) //local map 的半径
...
// local_map维护了一个半径为radius_m的局部地图
...
std::vector<bool> LocalMap<InputPointT, ClusteredPointT>::updatePose(const laser_slam::Pose& pose) {
  ...
  // 向removeIf传入函数：在指定半径范围外，返回true
    ...
    //remove反应是否移除点p，若距离过远（>radius_squared_m2_），或高程过高（> max_vertical_distance_m_），或高程过低（< min_vertical_distance_m_），则剔除点p。
    ...
    if (remove && p.ed_cluster_id != 0u)
      segment_ids_[p.ed_cluster_id] = kInvId; //kInvId= -2
...
std::vector<int> LocalMap<InputPointT, ClusteredPointT>::addPointsAndGetCreatedVoxels(
  ...
  // 将从上次执行SegMatchThread以来的所有scan存入merged_cloud
